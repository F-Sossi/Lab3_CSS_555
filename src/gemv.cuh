...
template<typename T>
__global__ void gemv_part2_ver22(const T * matrix, const T * vector, T * result, const unsigned int rows, const unsigned int col)
{
    // Calculate the thread index
    const unsigned int thread_index = threadIdx.x + blockIdx.x * blockDim.x;
    // Calculate the stride between threads
    ...
    for (unsigned int row = thread_index; row < rows; row += stride) {
        ...
        if (row < rows) { // add a conditional statement to check if the index is within bounds
    ...
    // Handle remaining indices separately
    if (thread_index == 0 && rows % stride != 0) {
        unsigned int row = rows - rows % stride;
